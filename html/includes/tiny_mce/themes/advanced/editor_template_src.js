...
function TinyMCE_advanced_getControlHTML(button_name)
{
	...
	switch (button_name)
	{
		...
		case "|":
		case "separator":
		return '<img border="0" src="{$themeurl}/images/spacer.gif" width="1" height="15" class="mceSeparatorLine">';

		case "spacer":
		return '<img border="0" src="{$themeurl}/images/spacer.gif" width="1" height="15" border="0" class="mceSeparatorLine" style="vertical-align: middle" />';
															  ...
															  undo_levels, visual_aid, any_selection)
{
	...
	if (pathElm)
	{
		...
		for (var i=path.length-1; i>=0; i--)
		{
			...
			if (tinyMCE.isMSIE)
			{
				html += '<a title="' + nodeData + '" href="javascript:tinyMCE.execInstanceCommand(\'' + editor_id + '\',\'mceSelectNodeDepth\',false,\'' + i + '\');" onmousedown="return false;" target="_self" class="mcePathItem">' + nodeName + '</a>';
			}
			else
			{
				html += '<a title="' + nodeData + '" href="javascript:tinyMCE.execInstanceCommand(\'' + editor_id + '\',\'mceSelectNodeDepth\',false,\'' + i + '\');" target="_self" class="mcePathItem">' + nodeName + '</a>';
			}
